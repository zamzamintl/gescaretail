...
class Partner(models.Model):
    ...
    property_product_pricelist = fields.Many2one(
        ...
        store=True,
        help="This pricelist will be used, instead of the default one, for\
         sales to the current partner")
    ...
    def _inverse_product_pricelist(self):
        ...
        actual = self.env['ir.property'].get(
            ...
            'res.partner', 'res.partner,%s' % self.id)

        # update at each change country, and so erase old pricelist
        if self.property_product_pricelist or (
            ...
            self.env['ir.property'].with_context(
                force_company=self.env.user.company_id.id).sudo().set_multi(
                'property_product_pricelist',
                self._name,
                {self.id:
                 self.property_product_pricelist or default_for_country.id},
